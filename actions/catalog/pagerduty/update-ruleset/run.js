...
const run = async (input) => {
  const { BUILDABLE_PAGERDUTY_API_KEY, accept, contentType, id, ruleset } = input;
 ...
 */
const verifyInput = ({ BUILDABLE_PAGERDUTY_API_KEY, accept, contentType, id, ruleset }) => {
  const ERRORS = {
    ...
    INVALID_RULESET: "A valid ruleset field (object) was not provided in the input.",
  };
  ...
  if (typeof ruleset !== "object") throw new Error(ERRORS.INVALID_RULESET);
};
