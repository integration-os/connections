...
const run = async (input) => {
  const { BUILDABLE_PAGERDUTY_API_KEY, accept, contentType, id, rule_id } = input;
 ...
 */
const verifyInput = ({ BUILDABLE_PAGERDUTY_API_KEY, accept, contentType, id, rule_id }) => {
  const ERRORS = {
    ...
    INVALID_RULE_ID: "A valid rule_id field (string) was not provided in the input.",
  };
  ...
  if (typeof rule_id !== "string") throw new Error(ERRORS.INVALID_RULE_ID);
};
