...
const run = async (input) => {
  const { BUILDABLE_PAGERDUTY_API_KEY, accept, contentType, id, from, source_incidents } = input;
...
const verifyInput = ({
  ...
  const ERRORS = {
    ...
    INVALID_SOURCE_INCIDENTS:
      "A valid source_incidents field (object) was not provided in the input.",
  };
  ...
  if (typeof source_incidents !== "object") throw new Error(ERRORS.INVALID_SOURCE_INCIDENTS);
};
