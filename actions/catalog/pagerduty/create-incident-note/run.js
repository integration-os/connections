...
const run = async (input) => {
  const { BUILDABLE_PAGERDUTY_API_KEY, accept, contentType, id, from, note } = input;
 ...
 */
const verifyInput = ({ BUILDABLE_PAGERDUTY_API_KEY, accept, contentType, id, from, note }) => {
  const ERRORS = {
    ...
    INVALID_NOTE: "A valid note field (object) was not provided in the input.",
  };
  ...
  if (typeof note !== "object") throw new Error(ERRORS.INVALID_NOTE);
};
