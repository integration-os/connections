...
const run = async (input) => {
  ...
  try {
    const { data } = await axios({
      ...
      data: { status_update_notification_rule },
    });
...
const verifyInput = ({
  ...
  const ERRORS = {
    ...
    INVALID_STATUS_UPDATE_NOTIFICATION_RULE:
      "A valid status_update_notification_rule field (object) was not provided in the input.",
  };
  ...
  if (typeof status_update_notification_rule !== "object")
    throw new Error(ERRORS.INVALID_STATUS_UPDATE_NOTIFICATION_RULE);
};
