...
const run = async (input) => {
  const { BUILDABLE_PAGERDUTY_API_KEY, accept, contentType, id, include, urgency } = input;
  ...
  try {
    const { data } = await axios({
      method: "get",
      url: `https://api.pagerduty.com/users/${id}/notification_rules`,
      headers: { Authorization: `Token token= ${BUILDABLE_PAGERDUTY_API_KEY}` },
      params: { ...(include ? { "include[]": include } : {}), ...(urgency ? { urgency } : {}) },
    });
