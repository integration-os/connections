...
const run = async (input) => {
  const { BUILDABLE_PAGERDUTY_API_KEY, accept, contentType, entity_type, id, add, remove } = input;
 ...
 */
const verifyInput = ({ BUILDABLE_PAGERDUTY_API_KEY, accept, contentType, entity_type, id }) => {
  const ERRORS = {
    ...
    INVALID_ENTITY_TYPE: "A valid entity_type field (string) was not provided in the input.",
  ...
  if (typeof entity_type !== "string") throw new Error(ERRORS.INVALID_ENTITY_TYPE);
