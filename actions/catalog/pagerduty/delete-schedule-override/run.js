...
const run = async (input) => {
  const { BUILDABLE_PAGERDUTY_API_KEY, accept, contentType, id, override_id } = input;
  ...
  try {
    const { data } = await axios({
      method: "delete",
      url: `https://api.pagerduty.com/schedules/${id}/overrides/${override_id}`,
 ...
 */
const verifyInput = ({ BUILDABLE_PAGERDUTY_API_KEY, accept, contentType, id, override_id }) => {
  const ERRORS = {
    ...
    INVALID_OVERRIDE_ID: "A valid override_id field (string) was not provided in the input.",
  };
  ...
  if (typeof override_id !== "string") throw new Error(ERRORS.INVALID_OVERRIDE_ID);
};
