...
const run = async (input) => {
  const { BUILDABLE_PAGERDUTY_API_KEY, accept, contentType, from, user } = input;
 ...
 */
const verifyInput = ({ BUILDABLE_PAGERDUTY_API_KEY, accept, contentType, from, user }) => {
  const ERRORS = {
    ...
    INVALID_USER: "A valid user field (object) was not provided in the input.",
  };
  ...
  if (typeof user !== "object") throw new Error(ERRORS.INVALID_USER);
};
