...
const run = async (input) => {
  const { BUILDABLE_PAGERDUTY_API_KEY, accept, contentType, tag } = input;
 ...
 */
const verifyInput = ({ BUILDABLE_PAGERDUTY_API_KEY, accept, contentType, tag }) => {
  const ERRORS = {
    ...
    INVALID_TAG: "A valid tag field (object) was not provided in the input.",
  };
  ...
  if (typeof tag !== "object") throw new Error(ERRORS.INVALID_TAG);
};
