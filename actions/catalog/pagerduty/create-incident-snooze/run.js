...
const run = async (input) => {
  const { BUILDABLE_PAGERDUTY_API_KEY, accept, contentType, id, from, duration } = input;
  ...
  try {
    const { data } = await axios({
      method: "post",
      url: `https://api.pagerduty.com/incidents/${id}/snooze`,
 ...
 */
const verifyInput = ({ BUILDABLE_PAGERDUTY_API_KEY, accept, contentType, id, from, duration }) => {
  const ERRORS = {
