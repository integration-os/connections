...
const run = async (input) => {
  const { BUILDABLE_PAGERDUTY_API_KEY, accept, contentType, id, team } = input;
 ...
 */
const verifyInput = ({ BUILDABLE_PAGERDUTY_API_KEY, accept, contentType, id, team }) => {
  const ERRORS = {
    ...
    INVALID_TEAM: "A valid team field (object) was not provided in the input.",
  };
  ...
  if (typeof team !== "object") throw new Error(ERRORS.INVALID_TEAM);
};
