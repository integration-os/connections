...
const run = async (input) => {
  const { BUILDABLE_PAGERDUTY_API_KEY, accept, contentType, id, escalation_policy_id } = input;
...
const verifyInput = ({
  ...
  const ERRORS = {
    ...
    INVALID_ESCALATION_POLICY_ID:
      "A valid escalation_policy_id field (string) was not provided in the input.",
  };
  ...
  if (typeof escalation_policy_id !== "string")
    throw new Error(ERRORS.INVALID_ESCALATION_POLICY_ID);
};
