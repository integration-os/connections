...
const run = async (input) => {
  const { BUILDABLE_PAGERDUTY_API_KEY, accept, id, subscribers } = input;
  ...
  try {
    const { data } = await axios({
      method: "post",
      url: `https://api.pagerduty.com/incidents/${id}/status_updates/unsubscribe`,
 ...
 */
const verifyInput = ({ BUILDABLE_PAGERDUTY_API_KEY, accept, id, subscribers }) => {
  const ERRORS = {
    ...
    INVALID_SUBSCRIBERS: "A valid subscribers field (object) was not provided in the input.",
  };
  ...
  if (typeof subscribers !== "object") throw new Error(ERRORS.INVALID_SUBSCRIBERS);
};
