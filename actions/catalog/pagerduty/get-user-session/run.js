...
const run = async (input) => {
  const { BUILDABLE_PAGERDUTY_API_KEY, accept, contentType, id, type, session_id } = input;
  ...
  try {
    const { data } = await axios({
      method: "get",
      url: `https://api.pagerduty.com/users/${id}/sessions/${type}/${session_id}`,
...
const verifyInput = ({
  ...
  const ERRORS = {
    ...
    INVALID_SESSION_ID: "A valid session_id field (string) was not provided in the input.",
  };
  ...
  if (typeof session_id !== "string") throw new Error(ERRORS.INVALID_SESSION_ID);
};
