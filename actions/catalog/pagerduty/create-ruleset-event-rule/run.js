...
const run = async (input) => {
  const { BUILDABLE_PAGERDUTY_API_KEY, accept, contentType, id, rule } = input;
  ...
  try {
    const { data } = await axios({
      method: "post",
      url: `https://api.pagerduty.com/rulesets/${id}/rules`,
 ...
 */
const verifyInput = ({ BUILDABLE_PAGERDUTY_API_KEY, accept, contentType, id, rule }) => {
  const ERRORS = {
    ...
    INVALID_RULE: "A valid rule field (object) was not provided in the input.",
  };
  ...
  if (typeof rule !== "object") throw new Error(ERRORS.INVALID_RULE);
};
