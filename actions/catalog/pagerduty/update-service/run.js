...
const run = async (input) => {
  const { BUILDABLE_PAGERDUTY_API_KEY, accept, contentType, id, service } = input;
 ...
 */
const verifyInput = ({ BUILDABLE_PAGERDUTY_API_KEY, accept, contentType, id, service }) => {
  const ERRORS = {
    ...
    INVALID_SERVICE: "A valid service field (object) was not provided in the input.",
  };
  ...
  if (typeof service !== "object") throw new Error(ERRORS.INVALID_SERVICE);
};
