...
const run = async (input) => {
  const { BUILDABLE_PAGERDUTY_API_KEY, accept, contentType, id, integration_id, integration } =
    input;
...
const verifyInput = ({
  ...
  const ERRORS = {
    ...
    INVALID_INTEGRATION_ID: "A valid integration_id field (string) was not provided in the input.",
    INVALID_INTEGRATION: "A valid integration field (object) was not provided in the input.",
  };
  ...
  if (typeof integration_id !== "string") throw new Error(ERRORS.INVALID_INTEGRATION_ID);
  if (typeof integration !== "object") throw new Error(ERRORS.INVALID_INTEGRATION);
};
