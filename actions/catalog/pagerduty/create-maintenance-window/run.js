...
const run = async (input) => {
  const { BUILDABLE_PAGERDUTY_API_KEY, accept, contentType, from, maintenance_window } = input;
...
const verifyInput = ({
  ...
  const ERRORS = {
    ...
    INVALID_MAINTENANCE_WINDOW:
      "A valid maintenance_window field (object) was not provided in the input.",
  };
  ...
  if (typeof maintenance_window !== "object") throw new Error(ERRORS.INVALID_MAINTENANCE_WINDOW);
};
