...
const run = async (input) => {
  const { BUILDABLE_PAGERDUTY_API_KEY, accept, contentType, id, orchestration } = input;
 ...
 */
const verifyInput = ({ BUILDABLE_PAGERDUTY_API_KEY, accept, contentType, id, orchestration }) => {
  const ERRORS = {
    ...
    INVALID_ORCHESTRATION: "A valid orchestration field (object) was not provided in the input.",
  };
  ...
  if (typeof orchestration !== "object") throw new Error(ERRORS.INVALID_ORCHESTRATION);
};
