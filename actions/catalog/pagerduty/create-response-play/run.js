...
const run = async (input) => {
  const { BUILDABLE_PAGERDUTY_API_KEY, accept, contentType, from, response_play } = input;
 ...
 */
const verifyInput = ({ BUILDABLE_PAGERDUTY_API_KEY, accept, contentType, from, response_play }) => {
  const ERRORS = {
    ...
    INVALID_RESPONSE_PLAY: "A valid response_play field (object) was not provided in the input.",
  };
  ...
  if (typeof response_play !== "object") throw new Error(ERRORS.INVALID_RESPONSE_PLAY);
};
