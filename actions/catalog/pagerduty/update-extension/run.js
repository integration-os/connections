...
const run = async (input) => {
  const { BUILDABLE_PAGERDUTY_API_KEY, accept, contentType, id, extension } = input;
 ...
 */
const verifyInput = ({ BUILDABLE_PAGERDUTY_API_KEY, accept, contentType, id, extension }) => {
  const ERRORS = {
    ...
    INVALID_EXTENSION: "A valid extension field (object) was not provided in the input.",
  };
  ...
  if (typeof extension !== "object") throw new Error(ERRORS.INVALID_EXTENSION);
};
