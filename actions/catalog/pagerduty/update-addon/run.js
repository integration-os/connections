...
const run = async (input) => {
  const { BUILDABLE_PAGERDUTY_API_KEY, accept, contentType, id, addon } = input;
 ...
 */
const verifyInput = ({ BUILDABLE_PAGERDUTY_API_KEY, accept, contentType, id, addon }) => {
  const ERRORS = {
    ...
    INVALID_ADDON: "A valid addon field (object) was not provided in the input.",
  };
  ...
  if (typeof addon !== "object") throw new Error(ERRORS.INVALID_ADDON);
};
