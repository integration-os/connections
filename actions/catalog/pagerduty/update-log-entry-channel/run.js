...
const run = async (input) => {
  const { BUILDABLE_PAGERDUTY_API_KEY, accept, contentType, id, from, channel } = input;
  ...
  try {
    const { data } = await axios({
      method: "put",
      url: `https://api.pagerduty.com/log_entries/${id}/channel`,
 ...
 */
const verifyInput = ({ BUILDABLE_PAGERDUTY_API_KEY, accept, contentType, id, from, channel }) => {
  const ERRORS = {
    ...
    INVALID_CHANNEL: "A valid channel field (object) was not provided in the input.",
  };
  ...
  if (typeof channel !== "object") throw new Error(ERRORS.INVALID_CHANNEL);
};
