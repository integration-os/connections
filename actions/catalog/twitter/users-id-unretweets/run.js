...
const run = async (input) => {
  const { BUILDABLE_TWITTER_BEARER_TOKEN, id, source_tweet_id } = input;
 ...
 */
const verifyInput = ({ BUILDABLE_TWITTER_BEARER_TOKEN, id, source_tweet_id }) => {
  const ERRORS = {
  ...
  if (typeof source_tweet_id !== "string") throw new Error(ERRORS.INVALID_SOURCE_TWEET_ID);
};
