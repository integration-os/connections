...
const run = async (input) => {
  const { BUILDABLE_STRIPE_API_KEY, session, ending_before, expand, limit, starting_after } = input;
  ...
  try {
    const { data } = await axios({
      method: "get",
      url: `https://api.stripe.com/v1/checkout/sessions/${session}/line_items`,
      headers: {
