...
const run = async (input) => {
  const { BUILDABLE_TWILIO_ACCOUNT_SID, BUILDABLE_TWILIO_AUTH_TOKEN, domainSid, pageSize } = input;
  ...
  try {
    const { data } = await axios({
      method: "get",
      url: `https://api.twilio.com/2010-04-01/Accounts/${BUILDABLE_TWILIO_ACCOUNT_SID}/SIP/Domains/${domainSid}/Auth/Registrations/CredentialListMappings.json`,
 ...
 */
const verifyInput = ({ BUILDABLE_TWILIO_ACCOUNT_SID, BUILDABLE_TWILIO_AUTH_TOKEN, domainSid }) => {
  const ERRORS = {
