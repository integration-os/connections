...
const run = async (input) => {
  const { BUILDABLE_CIRCLECI_PERSONAL_API_KEY, projectSlug, fingerprint } = input;
  ...
  try {
    const { data } = await axios({
      method: "delete",
      url: `https://circleci.com/api/v2/project/${projectSlug}/checkout-key/${fingerprint}`,
 ...
 */
const verifyInput = ({ BUILDABLE_CIRCLECI_PERSONAL_API_KEY, projectSlug, fingerprint }) => {
  const ERRORS = {
    ...
    INVALID_FINGERPRINT: "A valid fingerprint field (string) was not provided in the input.",
  };
  ...
  if (typeof fingerprint !== "string") throw new Error(ERRORS.INVALID_FINGERPRINT);
};
