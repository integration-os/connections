...
const run = async (input) => {
  const { BUILDABLE_CIRCLECI_PERSONAL_API_KEY, projectSlug, pipelineNumber } = input;
  ...
  try {
    const { data } = await axios({
      method: "get",
      url: `https://circleci.com/api/v2/project/${projectSlug}/pipeline/${pipelineNumber}`,
 ...
 */
const verifyInput = ({ BUILDABLE_CIRCLECI_PERSONAL_API_KEY, projectSlug, pipelineNumber }) => {
  const ERRORS = {
    ...
    INVALID_PIPELINE_NUMBER: "A valid pipelineNumber field (object) was not provided in the input.",
  };
  ...
  if (typeof pipelineNumber !== "object") throw new Error(ERRORS.INVALID_PIPELINE_NUMBER);
};
