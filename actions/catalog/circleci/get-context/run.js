...
const run = async (input) => {
  const { BUILDABLE_CIRCLECI_PERSONAL_API_KEY, contextId } = input;
  ...
  try {
    const { data } = await axios({
      method: "get",
      url: `https://circleci.com/api/v2/context/${contextId}`,
 ...
 */
const verifyInput = ({ BUILDABLE_CIRCLECI_PERSONAL_API_KEY, contextId }) => {
  const ERRORS = {
