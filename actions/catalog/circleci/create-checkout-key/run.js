...
const run = async (input) => {
  const { BUILDABLE_CIRCLECI_PERSONAL_API_KEY, projectSlug, type } = input;
  ...
  try {
    const { data } = await axios({
      method: "post",
      url: `https://circleci.com/api/v2/project/${projectSlug}/checkout-key`,
 ...
 */
const verifyInput = ({ BUILDABLE_CIRCLECI_PERSONAL_API_KEY, projectSlug, type }) => {
  const ERRORS = {
