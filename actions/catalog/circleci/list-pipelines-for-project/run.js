...
const run = async (input) => {
  const { BUILDABLE_CIRCLECI_PERSONAL_API_KEY, projectSlug, branch, pageToken } = input;
  ...
  try {
    const { data } = await axios({
      method: "get",
      url: `https://circleci.com/api/v2/project/${projectSlug}/pipeline`,
      auth: { username: BUILDABLE_CIRCLECI_PERSONAL_API_KEY },
      params: { ...(branch ? { branch } : {}), ...(pageToken ? { "page-token": pageToken } : {}) },
    });
