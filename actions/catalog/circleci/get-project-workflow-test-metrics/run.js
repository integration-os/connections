...
const run = async (input) => {
  const { BUILDABLE_CIRCLECI_PERSONAL_API_KEY, projectSlug, workflowName, branch, allBranches } =
    input;
  ...
  try {
    const { data } = await axios({
      method: "get",
      url: `https://circleci.com/api/v2/insights/${projectSlug}/workflows/${workflowName}/test-metrics`,
