...
const run = async (input) => {
  const { BUILDABLE_CIRCLECI_PERSONAL_API_KEY, projectSlug, workflowName } = input;
  ...
  try {
    const { data } = await axios({
      method: "get",
      url: `https://circleci.com/api/v2/insights/${projectSlug}/branches`,
      auth: { username: BUILDABLE_CIRCLECI_PERSONAL_API_KEY },
      params: { ...(workflowName ? { "workflow-name": workflowName } : {}) },
    });
