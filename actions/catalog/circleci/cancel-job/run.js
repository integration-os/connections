...
const run = async (input) => {
  const { BUILDABLE_CIRCLECI_PERSONAL_API_KEY, jobNumber, projectSlug } = input;
  ...
  try {
    const { data } = await axios({
      method: "post",
      url: `https://circleci.com/api/v2/project/${projectSlug}/job/${jobNumber}/cancel`,
 ...
 */
const verifyInput = ({ BUILDABLE_CIRCLECI_PERSONAL_API_KEY, jobNumber, projectSlug }) => {
  const ERRORS = {
    ...
    INVALID_JOB_NUMBER: "A valid jobNumber field (object) was not provided in the input.",
  ...
  if (typeof jobNumber !== "object") throw new Error(ERRORS.INVALID_JOB_NUMBER);
