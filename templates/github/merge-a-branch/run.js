 ...
 * @access    open
 * @license   MIT
 * @docs      https://docs.github.com/enterprise-server@3.3/rest/reference/repos#merge-a-branch
 *
 * ----------------------------------------------------------------------------------------------------
 */
...
const run = async (input) => {
  const { GITHUB_API_USERNAME, GITHUB_API_TOKEN, owner, repo, base, head, commit_message } = input;
  ...
  try {
    const { data } = await axios({
      ...
      params: {},
      data: { head, base, ...(commit_message ? { commit_message } : {}) },
    });
 ...
 */
const verifyInput = ({ owner, repo, head, base }) => {
  const ERRORS = {
