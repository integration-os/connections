/**
 * ----------------------------------------------------------------------------------------------------
 * Update a Code Scanning Alert [Run]
 *
 * @description - Update a Code Scanning Alert using the GitHub API
 *
 * @author    Buildable Technologies Inc.
 * @access    open
 * @license   MIT
 * @docs      https://docs.github.com/enterprise-server@3.3/rest/reference/code-scanning#update-a-code-scanning-alert
 *
 * ----------------------------------------------------------------------------------------------------
 */
...
const run = async (input) => {
  ...
  try {
    const { data } = await axios({
      ...
      params: {},
      data: { state, ...(dismissed_reason ? { dismissed_reason } : {}) },
    });
