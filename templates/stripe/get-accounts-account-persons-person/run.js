...
const run = async (input) => {
  const { BUILDABLE_STRIPE_API_KEY, account, person, expand } = input;
  ...
  try {
    const { data } = await axios({
      method: "get",
      url: `https://api.stripe.com/v1/accounts/${account}/persons/${person}`,
      headers: {
 ...
 */
const verifyInput = ({ BUILDABLE_STRIPE_API_KEY, account, person }) => {
  const ERRORS = {
