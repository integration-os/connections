...
const run = async (input) => {
  const { BUILDABLE_STRIPE_API_KEY, name, payload, scope, expand } = input;
  ...
  try {
    const { data } = await axios({
      ...
      data: qs.stringify({ name, payload, scope, ...(expand ? { expand } : {}) }),
    });
 ...
 */
const verifyInput = ({ BUILDABLE_STRIPE_API_KEY, name, payload, scope }) => {
  const ERRORS = {
