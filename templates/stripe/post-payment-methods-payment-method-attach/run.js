...
const run = async (input) => {
  const { BUILDABLE_STRIPE_API_KEY, payment_method, customer, expand } = input;
  ...
  try {
    const { data } = await axios({
      ...
      data: qs.stringify({ customer, ...(expand ? { expand } : {}) }),
    });
 ...
 */
const verifyInput = ({ BUILDABLE_STRIPE_API_KEY, payment_method, customer }) => {
  const ERRORS = {
