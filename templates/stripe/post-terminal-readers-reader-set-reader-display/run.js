...
const run = async (input) => {
  const { BUILDABLE_STRIPE_API_KEY, reader, type, cart, expand } = input;
  ...
  try {
    const { data } = await axios({
      ...
      data: qs.stringify({ type, ...(cart ? { cart } : {}), ...(expand ? { expand } : {}) }),
    });
 ...
 */
const verifyInput = ({ BUILDABLE_STRIPE_API_KEY, reader, type }) => {
  const ERRORS = {
