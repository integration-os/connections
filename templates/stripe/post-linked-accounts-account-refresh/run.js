...
const run = async (input) => {
  const { BUILDABLE_STRIPE_API_KEY, account, features, expand } = input;
  ...
  try {
    const { data } = await axios({
      method: "post",
      url: `https://api.stripe.com/v1/linked_accounts/${account}/refresh`,
      headers: {
        Authorization: `Bearer ${BUILDABLE_STRIPE_API_KEY}`,
        "Content-Type": "application/x-www-form-urlencoded",
      },
      data: qs.stringify({ features, ...(expand ? { expand } : {}) }),
    });
 ...
 */
const verifyInput = ({ BUILDABLE_STRIPE_API_KEY, account, features }) => {
  const ERRORS = {
