/**
 * ----------------------------------------------------------------------------------------------------
 * Invalidate a Single Session for a User by Session_id [Run]
 *
 * @description - Invalidate a single session for a user by session_id using the Slack API
 *
...
const run = async (input) => {
  const { SLACK_ACCESS_TOKEN, session_id, team_id } = input;
 ...
 */
const verifyInput = ({ SLACK_ACCESS_TOKEN, session_id, team_id }) => {
  const ERRORS = {
