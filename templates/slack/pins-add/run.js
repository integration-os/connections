/**
 * ----------------------------------------------------------------------------------------------------
 * Pins an Item to a Channel. [Run]
 *
 * @description - Pins an item to a channel. using the Slack API
 *
...
const run = async (input) => {
  const { SLACK_ACCESS_TOKEN, channel, timestamp } = input;
  ...
  try {
    const { data } = await axios({
      ...
      data: qs.stringify({ channel, ...(timestamp ? { timestamp } : {}) }),
    });
