...
const run = async (input) => {
  const { BUILDABLE_SLACK_ACCESS_TOKEN, channel_ids, team_id, token } = input;
  ...
  try {
    const { data } = await axios({
      ...
      data: qs.stringify({ channel_ids, team_id, token }),
    });
 ...
 */
const verifyInput = ({ BUILDABLE_SLACK_ACCESS_TOKEN, channel_ids, team_id, token }) => {
  const ERRORS = {
