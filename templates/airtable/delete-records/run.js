 ...
 * Delete Records [Run]
 *
 * @description - Delete records from a table using the Airtable API
 *
...
const run = async (input) => {
  ...
  const url = getUrl({ endpoint, AIRTABLE_BASE_ID, tableName, records });
...
const getUrl = ({ endpoint, AIRTABLE_BASE_ID, tableName, records }) => {
  let url = `${endpoint}/${AIRTABLE_BASE_ID}/${tableName}`;

  const params = records.map((record) => `records[]=${record}`);
...
const verifyInput = ({ AIRTABLE_API_KEY, AIRTABLE_BASE_ID, endpoint, tableName, records }) => {
  const ERRORS = {
    ...
    INVALID_RECORDS: "A valid records field (array) was not provided in the input.",
  };
