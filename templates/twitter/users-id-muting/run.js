/**
 * ----------------------------------------------------------------------------------------------------
 * Returns User Objects that Are Muted by the Provided User ID [Run]
 *
 * @description - Returns user objects that are muted by the provided user id using the Twitter API
 *
...
const run = async (input) => {
  ...
  try {
    const { data } = await axios({
      method: "get",
      url: `https://api.twitter.com/2/users/${id}/muting`,
