/**
 * ----------------------------------------------------------------------------------------------------
 * Delete Recording Add on Result [Run]
 *
...
const run = async (input) => {
  const { TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, referenceSid, sid } = input;
  ...
  try {
    const { data } = await axios({
      method: "delete",
      url: `https://api.twilio.com/2010-04-01/Accounts/${TWILIO_ACCOUNT_SID}/Recordings/${referenceSid}/AddOnResults/${sid}.json`,
 ...
 */
const verifyInput = ({ TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, referenceSid, sid }) => {
  const ERRORS = {
