/**
 * ----------------------------------------------------------------------------------------------------
 * Delete Recording Transcription [Run]
 *
 * @description -
 *
...
const run = async (input) => {
  const { TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, recordingSid, sid } = input;
  ...
  try {
    const { data } = await axios({
      method: "delete",
      url: `https://api.twilio.com/2010-04-01/Accounts/${TWILIO_ACCOUNT_SID}/Recordings/${recordingSid}/Transcriptions/${sid}.json`,
 ...
 */
const verifyInput = ({ TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, recordingSid, sid }) => {
  const ERRORS = {
    ...
    INVALID_RECORDING_SID: "A valid recordingSid field (string) was not provided in the input.",
  ...
  if (typeof recordingSid !== "string") throw new Error(ERRORS.INVALID_RECORDING_SID);
