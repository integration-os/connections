/**
 * ----------------------------------------------------------------------------------------------------
 * Fetch Incoming Phone Number Assigned Add on Extension [Run]
 *
...
const run = async (input) => {
  const { TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, resourceSid, assignedAddOnSid, sid } = input;
  ...
  try {
    const { data } = await axios({
      method: "get",
      url: `https://api.twilio.com/2010-04-01/Accounts/${TWILIO_ACCOUNT_SID}/IncomingPhoneNumbers/${resourceSid}/AssignedAddOns/${assignedAddOnSid}/Extensions/${sid}.json`,
