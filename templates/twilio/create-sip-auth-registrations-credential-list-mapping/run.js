/**
 * ----------------------------------------------------------------------------------------------------
 * Create Sip Auth Registrations Credential List Mapping [Run]
 *
...
const run = async (input) => {
  const { TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, domainSid, credentialListSid } = input;
  ...
  try {
    const { data } = await axios({
      method: "post",
      url: `https://api.twilio.com/2010-04-01/Accounts/${TWILIO_ACCOUNT_SID}/SIP/Domains/${domainSid}/Auth/Registrations/CredentialListMappings.json`,
      headers: { "Content-Type": "application/x-www-form-urlencoded" },
      auth: { username: TWILIO_ACCOUNT_SID, password: TWILIO_AUTH_TOKEN },
      data: qs.stringify({ CredentialListSid: credentialListSid }),
    });
 ...
 */
const verifyInput = ({ TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, domainSid, credentialListSid }) => {
  const ERRORS = {
