...
const run = async (input) => {
  const { BUILDABLE_TWILIO_ACCOUNT_SID, BUILDABLE_TWILIO_AUTH_TOKEN, addressSid, pageSize } = input;
  ...
  try {
    const { data } = await axios({
      method: "get",
      url: `https://api.twilio.com/2010-04-01/Accounts/${BUILDABLE_TWILIO_ACCOUNT_SID}/Addresses/${addressSid}/DependentPhoneNumbers.json`,
 ...
 */
const verifyInput = ({ BUILDABLE_TWILIO_ACCOUNT_SID, BUILDABLE_TWILIO_AUTH_TOKEN, addressSid }) => {
  const ERRORS = {
    ...
    INVALID_ADDRESS_SID: "A valid addressSid field (string) was not provided in the input.",
  };
  ...
  if (typeof addressSid !== "string") throw new Error(ERRORS.INVALID_ADDRESS_SID);
};
