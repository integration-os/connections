...
const run = async (input) => {
  const { TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, credentialListSid, sid, password } = input;
  ...
  try {
    const { data } = await axios({
      method: "post",
      url: `https://api.twilio.com/2010-04-01/Accounts/${TWILIO_ACCOUNT_SID}/SIP/CredentialLists/${credentialListSid}/Credentials/${sid}.json`,
 ...
 */
const verifyInput = ({ TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, credentialListSid, sid }) => {
  const ERRORS = {
