/**
 * ----------------------------------------------------------------------------------------------------
 * List Recording Add on Result Payload [Run]
 *
...
const run = async (input) => {
  const { TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, referenceSid, addOnResultSid, pageSize } = input;
  ...
  try {
    const { data } = await axios({
      method: "get",
      url: `https://api.twilio.com/2010-04-01/Accounts/${TWILIO_ACCOUNT_SID}/Recordings/${referenceSid}/AddOnResults/${addOnResultSid}/Payloads.json`,
 ...
 */
const verifyInput = ({ TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, referenceSid, addOnResultSid }) => {
  const ERRORS = {
