/**
 * ----------------------------------------------------------------------------------------------------
 * Get Scrypta Spendable UTXO [Run]
 *
 * @description - Get scrypta spendable utxo using the Tatum API
 *
...
const run = async (input) => {
  const { TATUM_API_URL, TATUM_API_KEY, address, offset, pageSize } = input;
  ...
  try {
    const { data } = await axios({
      method: "get",
      url: `${TATUM_API_URL}/v3/scrypta/utxo/${address}`,
 ...
 */
const verifyInput = ({ TATUM_API_KEY, TATUM_API_URL, pageSize, offset, address }) => {
  const ERRORS = {
