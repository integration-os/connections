/**
 * ----------------------------------------------------------------------------------------------------
 * Get QTUM Transactions by Pagination [Run]
 *
 * @description - Get qtum transactions by pagination using the Tatum API
 *
...
const run = async (input) => {
  const { TATUM_API_URL, TATUM_API_KEY, offset, pageSize, address } = input;
  ...
  try {
    const { data } = await axios({
      method: "get",
      url: `${TATUM_API_URL}/v3/qtum/transactions/address/${address}`,
 ...
 */
const verifyInput = ({ TATUM_API_KEY, TATUM_API_URL, address, pageSize, offset }) => {
  const ERRORS = {
