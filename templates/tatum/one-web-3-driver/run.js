 ...
 * @access    open
 * @license   MIT
 * @docs      https://tatum.io/apidoc.php#operation/OneWeb3Driver
 *
 * ----------------------------------------------------------------------------------------------------
 */
...
const run = async (input) => {
  const { TATUM_API_URL, xApiKey, jsonrpc, method, params, id, shardID } =
    input;
  ...
  try {
    const { data } = await axios({
      method: "post",
      url: `${TATUM_API_URL}/v3/one/web3/${xApiKey}`,
