/**
 * ----------------------------------------------------------------------------------------------------
 * Generate Flow Address From Extended Public Key [Run]
 *
 * @description - Generate flow address from extended public key using the Tatum API
 *
 * @author    Buildable Technologies Inc.
 * @access    open
 * @license   MIT
 * @docs      https://tatum.io/apidoc.php#operation/FlowGenerateAddress
 *
 * ----------------------------------------------------------------------------------------------------
 */
...
const run = async (input) => {
  ...
  try {
    const { data } = await axios({
      method: "get",
      url: `${TATUM_API_URL}/v3/flow/address/${xpub}/${index}`,
