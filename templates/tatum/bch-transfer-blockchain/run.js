...
const run = async (input) => {
  const { BUILDABLE_TATUM_API_KEY, BUILDABLE_TATUM_API_URL, fromUTXO, to } = input;
  ...
  try {
    const { data } = await axios({
      method: "post",
      url: `${BUILDABLE_TATUM_API_URL}/v3/bcash/transaction`,
 ...
 */
const verifyInput = ({ BUILDABLE_TATUM_API_KEY, BUILDABLE_TATUM_API_URL, fromUTXO, to }) => {
  const ERRORS = {
