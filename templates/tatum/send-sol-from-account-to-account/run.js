/**
 * ----------------------------------------------------------------------------------------------------
 * Send SOL From Account to Account [Run]
 *
 * @description - Send sol from account to account using the Tatum API
 *
...
const run = async (input) => {
  const { TATUM_API_URL, TATUM_API_KEY, fromPrivateKey, amount, to, from } = input;
  ...
  try {
    const { data } = await axios({
      method: "post",
      url: `${TATUM_API_URL}/v3/solana/transaction`,
      headers: { "x-api-key": `${TATUM_API_KEY}` },
      params: {},
      data: { fromPrivateKey, amount, to, from },
    });
