...
const run = async (input) => {
  const { BUILDABLE_TATUM_API_KEY, BUILDABLE_TATUM_API_URL, chain, contractAddress, id } = input;
  ...
  try {
    const { data } = await axios({
      method: "get",
      url: `${BUILDABLE_TATUM_API_URL}/v3/blockchain/marketplace/listing/${chain}/${contractAddress}/listing/${id}`,
