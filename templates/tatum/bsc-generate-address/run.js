/**
 * ----------------------------------------------------------------------------------------------------
 * Generate BSC Account Address From Extended Public Key [Run]
 *
 * @description - Generate bsc account address from extended public key using the Tatum API
 *
 * @author    Buildable Technologies Inc.
 * @access    open
 * @license   MIT
 * @docs      https://tatum.io/apidoc.php#operation/BscGenerateAddress
 *
 * ----------------------------------------------------------------------------------------------------
 */
...
const run = async (input) => {
  ...
  try {
    const { data } = await axios({
      method: "get",
      url: `${TATUM_API_URL}/v3/bsc/address/${xpub}/${index}`,
