 ...
 * Add NFT Minter [Run]
 *
 * @description - Add nft minter using the Tatum API
 *
 * @author    Buildable Technologies Inc.
 * @access    open
 * @license   MIT
 * @docs      https://tatum.io/apidoc.php#operation/NftAddMinter
 *
 * ----------------------------------------------------------------------------------------------------
 */
...
const run = async (input) => {
  ...
  try {
    const { data } = await axios({
      method: "post",
      url: `${TATUM_API_URL}/v3/nft/mint/add`,
      headers: {
...
const verifyInput = ({
  ...
  const ERRORS = {
    ...
    INVALID_MINTER:
      "A valid minter field (string) was not provided in the input.",
