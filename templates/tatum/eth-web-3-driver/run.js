 ...
 * @access    open
 * @license   MIT
 * @docs      https://tatum.io/apidoc.php#operation/EthWeb3Driver
 *
 * ----------------------------------------------------------------------------------------------------
 */
...
const run = async (input) => {
  const { TATUM_API_URL, xApiKey, jsonrpc, method, params, id, testnetType } =
    input;
  ...
  try {
    const { data } = await axios({
      method: "post",
      url: `${TATUM_API_URL}/v3/ethereum/web3/${xApiKey}`,
      params: { ...(testnetType ? { testnetType } : {}) },
