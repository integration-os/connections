/**
 * ----------------------------------------------------------------------------------------------------
 * Get NFT Token Provenance Data [Run]
 *
 * @description - Get nft token provenance data using the Tatum API
 *
 * @author    Buildable Technologies Inc.
 * @access    open
 * @license   MIT
 * @docs      https://tatum.io/apidoc.php#operation/NftGetProvenanceDataErc721
 *
 * ----------------------------------------------------------------------------------------------------
 */
...
const run = async (input) => {
  const { TATUM_API_URL, TATUM_API_KEY, contractAddress, tokenId, chain, testnetType } = input;
  ...
  try {
    const { data } = await axios({
      method: "get",
      url: `${TATUM_API_URL}/v3/nft/provenance/${chain}/${contractAddress}/${tokenId}`,
      headers: {
 ...
 */
const verifyInput = ({ TATUM_API_KEY, TATUM_API_URL, chain, tokenId, contractAddress }) => {
  const ERRORS = {
