...
const run = async (input) => {
  const { BUILDABLE_TATUM_API_KEY, BUILDABLE_TATUM_API_URL, chain, address, testnetType } = input;
  ...
  try {
    const { data } = await axios({
      method: "get",
      url: `${BUILDABLE_TATUM_API_URL}/v3/multitoken/address/balance/${chain}/${address}`,
      headers: {
 ...
 */
const verifyInput = ({ BUILDABLE_TATUM_API_KEY, BUILDABLE_TATUM_API_URL, chain, address }) => {
  const ERRORS = {
