/**
 * ----------------------------------------------------------------------------------------------------
 * Get Multi Tokens by Address [Run]
 *
 * @description - Get multi tokens by address using the Tatum API
 *
...
const run = async (input) => {
  const { TATUM_API_KEY, TATUM_API_URL, chain, address, testnetType } = input;
  ...
  try {
    const { data } = await axios({
      method: "get",
      url: `${TATUM_API_URL}/v3/multitoken/address/balance/${chain}/${address}`,
      headers: {
 ...
 */
const verifyInput = ({ TATUM_API_KEY, TATUM_API_URL, chain, address }) => {
  const ERRORS = {
