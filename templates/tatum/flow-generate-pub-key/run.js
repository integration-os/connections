/**
 * ----------------------------------------------------------------------------------------------------
 * Generate Flow Public Key From Extended Public Key [Run]
 *
 * @description - Generate flow public key from extended public key using the Tatum API
 *
 * @author    Buildable Technologies Inc.
 * @access    open
 * @license   MIT
 * @docs      https://tatum.io/apidoc.php#operation/FlowGeneratePubKey
 *
 * ----------------------------------------------------------------------------------------------------
 */
...
const run = async (input) => {
  ...
  try {
    const { data } = await axios({
      method: "get",
      url: `${TATUM_API_URL}/v3/flow/pubkey/${xpub}/${index}`,
