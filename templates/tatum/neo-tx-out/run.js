...
const run = async (input) => {
  const { BUILDABLE_TATUM_API_KEY, BUILDABLE_TATUM_API_URL, txId, index } = input;
  ...
  try {
    const { data } = await axios({
      method: "get",
      url: `${BUILDABLE_TATUM_API_URL}/v3/neo/transaction/out/${txId}/${index}`,
 ...
 */
const verifyInput = ({ BUILDABLE_TATUM_API_KEY, BUILDABLE_TATUM_API_URL, txId, index }) => {
  const ERRORS = {
