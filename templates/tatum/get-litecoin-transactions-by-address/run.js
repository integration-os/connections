/**
 * ----------------------------------------------------------------------------------------------------
 * Get Litecoin Transactions by Address [Run]
 *
 * @description - Get litecoin transactions by address using the Tatum API
 *
 * @author    Buildable Technologies Inc.
 * @access    open
 * @license   MIT
 * @docs      https://tatum.io/apidoc.php#operation/LtcGetTxByAddress
 *
 * ----------------------------------------------------------------------------------------------------
 */
...
const run = async (input) => {
  const { TATUM_API_URL, TATUM_API_KEY, pageSize, address, offset } = input;
  ...
  try {
    const { data } = await axios({
      method: "get",
      url: `${TATUM_API_URL}/v3/litecoin/transaction/address/${address}`,
