/**
 * ----------------------------------------------------------------------------------------------------
 * Get Multi Token Token Metadata [Run]
 *
 * @description - Get multi token token metadata using the Tatum API
 *
 * @author    Buildable Technologies Inc.
 * @access    open
 * @license   MIT
 * @docs      https://tatum.io/apidoc.php#operation/MultiTokenGetMetadata
 *
 * ----------------------------------------------------------------------------------------------------
 */
...
const run = async (input) => {
  const { TATUM_API_URL, TATUM_API_KEY, contractAddress, token, chain, testnetType } = input;
  ...
  try {
    const { data } = await axios({
      method: "get",
      url: `${TATUM_API_URL}/v3/multitoken/metadata/${chain}/${contractAddress}/${token}`,
      headers: {
 ...
 */
const verifyInput = ({ TATUM_API_KEY, TATUM_API_URL, chain, token, contractAddress }) => {
  const ERRORS = {
