/**
 * ----------------------------------------------------------------------------------------------------
 * Get NFT Transactions by Address [Run]
 *
 * @description - Get nft transactions by address using the Tatum API
 *
 * @author    Buildable Technologies Inc.
 * @access    open
 * @license   MIT
 * @docs      https://tatum.io/apidoc.php#operation/NftGetTransactionByAddress
 *
 * ----------------------------------------------------------------------------------------------------
 */
...
const run = async (input) => {
  ...
  try {
    const { data } = await axios({
      method: "get",
      url: `${TATUM_API_URL}/v3/nft/transaction/${chain}/${address}/${tokenAddress}`,
