/**
 * ----------------------------------------------------------------------------------------------------
 * Estimate Polygon Transaction Fees [Run]
 *
 * @description - Estimate polygon transaction fees using the Tatum API
 *
 * @author    Buildable Technologies Inc.
 * @access    open
 * @license   MIT
 * @docs      https://tatum.io/apidoc.php#operation/PolygonEstimateGas
 *
 * ----------------------------------------------------------------------------------------------------
 */
...
const run = async (input) => {
  const { TATUM_API_URL, TATUM_API_KEY, amount, to, from, data } = input;
  ...
  try {
    const { data: _data } = await axios({
      method: "post",
      url: `${TATUM_API_URL}/v3/polygon/gas`,
      headers: { "x-api-key": `${TATUM_API_KEY}` },
      params: {},
      data: { amount, to, from, ...(data ? { data } : {}) },
    });
 ...
 */
const verifyInput = ({ TATUM_API_KEY, TATUM_API_URL, amount, to, from }) => {
  const ERRORS = {
