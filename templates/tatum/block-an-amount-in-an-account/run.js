...
const run = async (input) => {
  const { TATUM_API_URL, TATUM_API_KEY, type, amount, id, description } = input;
  ...
  try {
    const { data } = await axios({
      ...
      params: {},
      data: { type, amount, ...(description ? { description } : {}) },
    });
 ...
 */
const verifyInput = ({ TATUM_API_KEY, TATUM_API_URL, id, type, amount }) => {
  const ERRORS = {
