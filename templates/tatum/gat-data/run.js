 ...
 * Gat Data [Run]
 *
 * @description - Gat data using the Tatum API
 *
...
const run = async (input) => {
  const { TATUM_API_URL, TATUM_API_KEY, key, fabricEndpoint } = input;
  ...
  try {
    const { data } = await axios({
      method: "get",
      url: `${TATUM_API_URL}/v3/fabric/data/${key}`,
      headers: {
 ...
 */
const verifyInput = ({ TATUM_API_KEY, fabricEndpoint, TATUM_API_URL, key }) => {
  const ERRORS = {
