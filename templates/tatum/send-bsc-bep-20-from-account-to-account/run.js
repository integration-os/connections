/**
 * ----------------------------------------------------------------------------------------------------
 * Send BSC / BEP20 From Account to Account [Run]
 *
 * @description - Send bsc / bep20 from account to account using the Tatum API
 *
 * @author    Buildable Technologies Inc.
 * @access    open
 * @license   MIT
 * @docs      https://tatum.io/apidoc.php#operation/BscBlockchainTransfer
 *
 * ----------------------------------------------------------------------------------------------------
 */
...
const run = async (input) => {
  const { TATUM_API_URL, TATUM_API_KEY, fromPrivateKey, amount, currency, to, data, nonce, fee } =
    input;
  ...
  try {
    const { data: _data } = await axios({
      method: "post",
      url: `${TATUM_API_URL}/v3/bsc/transaction`,
