 ...
 * @access    open
 * @license   MIT
 * @docs      https://tatum.io/apidoc.php#operation/KcsBlockchainSmartContractInvocation
 *
 * ----------------------------------------------------------------------------------------------------
 */
...
const run = async (input) => {
  const { TATUM_API_URL, TATUM_API_KEY, params, methodABI, methodName, contractAddress } = input;
  ...
  try {
    const { data } = await axios({
      method: "post",
      url: `${TATUM_API_URL}/v3/kcs/smartcontract`,
      headers: { "x-api-key": `${TATUM_API_KEY}` },
      params: {},
      data: { params, methodABI, methodName, contractAddress },
    });
