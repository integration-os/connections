/**
 * ----------------------------------------------------------------------------------------------------
 * Send Bitcoin From Tatum Account to Address [Run]
 *
 * @description - Send bitcoin from tatum account to address using the Tatum API
 *
 * @author    Buildable Technologies Inc.
 * @access    open
 * @license   MIT
 * @docs      https://tatum.io/apidoc.php#operation/BtcTransfer
 *
 * ----------------------------------------------------------------------------------------------------
 */
...
const run = async (input) => {
  ...
  try {
    const { data } = await axios({
      method: "post",
      url: `${TATUM_API_URL}/v3/offchain/bitcoin/transfer`,
