/**
 * ----------------------------------------------------------------------------------------------------
 * Send ONE / HRM20 From Account to Account [Run]
 *
 * @description - Send one / hrm20 from account to account using the Tatum API
 *
 * @author    Buildable Technologies Inc.
 * @access    open
 * @license   MIT
 * @docs      https://tatum.io/apidoc.php#operation/OneBlockchainTransfer
 *
 * ----------------------------------------------------------------------------------------------------
 */
...
const run = async (input) => {
  ...
  try {
    const { data: _data } = await axios({
      method: "post",
      url: `${TATUM_API_URL}/v3/one/transaction`,
 ...
 */
const verifyInput = ({ TATUM_API_KEY, TATUM_API_URL, fromPrivateKey, amount, to, currency }) => {
  const ERRORS = {
