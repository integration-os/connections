/**
 * ----------------------------------------------------------------------------------------------------
 * Get Ethereum Transactions by Address [Run]
 *
 * @description - Get ethereum transactions by address using the Tatum API
 *
...
const run = async (input) => {
  const { TATUM_API_URL, TATUM_API_KEY, pageSize, address, testnetType, offset, from, to, sort } =
    input;
  ...
  try {
    const { data } = await axios({
      method: "get",
      url: `${TATUM_API_URL}/v3/ethereum/account/transaction/${address}`,
      headers: {
