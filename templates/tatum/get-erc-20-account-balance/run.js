/**
 * ----------------------------------------------------------------------------------------------------
 * Get ERC20 Account Balance [Run]
 *
 * @description - Get erc20 account balance using the Tatum API
 *
 * @author    Buildable Technologies Inc.
 * @access    open
 * @license   MIT
 * @docs      https://tatum.io/apidoc.php#operation/Erc20GetBalance
 *
 * ----------------------------------------------------------------------------------------------------
 */
...
const run = async (input) => {
  const { TATUM_API_URL, TATUM_API_KEY, contractAddress, address, chain, testnetType } = input;
  ...
  try {
    const { data } = await axios({
      method: "get",
      url: `${TATUM_API_URL}/v3/blockchain/token/balance/${chain}/${contractAddress}/${address}`,
      headers: {
 ...
 */
const verifyInput = ({ TATUM_API_KEY, TATUM_API_URL, chain, address, contractAddress }) => {
  const ERRORS = {
