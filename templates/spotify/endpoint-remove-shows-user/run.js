/**
 * ----------------------------------------------------------------------------------------------------
 * Remove User's Saved Shows [Run]
 *
 * @description - Remove user's saved shows using the Spotify API
 *
 * @author    Buildable Technologies Inc.
 * @access    open
 * @license   MIT
 * @docs      https://developer.spotify.com/documentation/web-api/reference/#endpoint-remove-shows-user
 *
 * ----------------------------------------------------------------------------------------------------
 */
...
const run = async (input) => {
  const { SPOTIFY_CLIENT_ID, SPOTIFY_CLIENT_SECRET, SPOTIFY_BASE_URI, ids, market, ids } = input;
  ...
  try {
    ...
    const { data } = await axios({
      method: "delete",
      url: `${SPOTIFY_BASE_URI}/me/shows`,
      headers: {
        Authorization: `Bearer ${access_token}`,
      },
      params: { ids, ...(market ? { market } : {}) },
      data: { ...(ids ? { ids } : {}) },
    });
