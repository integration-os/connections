/**
 * ----------------------------------------------------------------------------------------------------
 * Set Volume For User's Playback [Run]
 *
 * @description - Set volume for user's playback using the Spotify API
 *
 * @author    Buildable Technologies Inc.
 * @access    open
 * @license   MIT
 * @docs      https://developer.spotify.com/documentation/web-api/reference/#endpoint-set-volume-for-users-playback
 *
 * ----------------------------------------------------------------------------------------------------
 */
...
const run = async (input) => {
  const { SPOTIFY_CLIENT_ID, SPOTIFY_CLIENT_SECRET, SPOTIFY_BASE_URI, volume_percent, device_id } =
    input;
  ...
  try {
    ...
    const { data } = await axios({
      method: "put",
      url: `${SPOTIFY_BASE_URI}/me/player/volume`,
      headers: {
...
const verifyInput = ({
  ...
  const ERRORS = {
    ...
    INVALID_VOLUME_PERCENT: "A valid volume_percent field (number) was not provided in the input.",
  };
  ...
  if (typeof volume_percent !== "number") throw new Error(ERRORS.INVALID_VOLUME_PERCENT);
};
