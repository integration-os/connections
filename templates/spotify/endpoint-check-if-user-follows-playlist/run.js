/**
 * ----------------------------------------------------------------------------------------------------
 * Check if Users Follow a Playlist [Run]
 *
 * @description - Check if users follow a playlist using the Spotify API
 *
 * @author    Buildable Technologies Inc.
 * @access    open
 * @license   MIT
 * @docs      https://developer.spotify.com/documentation/web-api/reference/#endpoint-check-if-user-follows-playlist
 *
 * ----------------------------------------------------------------------------------------------------
 */
...
const run = async (input) => {
  const { SPOTIFY_CLIENT_ID, SPOTIFY_CLIENT_SECRET, SPOTIFY_BASE_URI, playlist_id, ids } = input;
  ...
  try {
    ...
    const { data } = await axios({
      method: "get",
      url: `${SPOTIFY_BASE_URI}/playlists/${playlist_id}/followers/contains`,
      headers: {
...
const verifyInput = ({
  ...
  const ERRORS = {
    ...
    INVALID_PLAYLIST_ID: "A valid playlist_id field (string) was not provided in the input.",
  ...
  if (typeof playlist_id !== "string") throw new Error(ERRORS.INVALID_PLAYLIST_ID);
