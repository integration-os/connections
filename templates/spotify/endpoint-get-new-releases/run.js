/**
 * ----------------------------------------------------------------------------------------------------
 * Get All New Releases [Run]
 *
 * @description - Get all new releases using the Spotify API
 *
 * @author    Buildable Technologies Inc.
 * @access    open
 * @license   MIT
 * @docs      https://developer.spotify.com/documentation/web-api/reference/#endpoint-get-new-releases
 *
 * ----------------------------------------------------------------------------------------------------
 */
...
const run = async (input) => {
  const { SPOTIFY_CLIENT_ID, SPOTIFY_CLIENT_SECRET, SPOTIFY_BASE_URI, country, limit, offset } =
    input;
  ...
  try {
    ...
    const { data } = await axios({
      method: "get",
      url: `${SPOTIFY_BASE_URI}/browse/new-releases`,
      headers: {
