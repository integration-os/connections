/**
 * ----------------------------------------------------------------------------------------------------
 * Set Repeat Mode On User’s Playback [Run]
 *
 * @description - Set repeat mode on user’s playback using the Spotify API
 *
 * @author    Buildable Technologies Inc.
 * @access    open
 * @license   MIT
 * @docs      https://developer.spotify.com/documentation/web-api/reference/#endpoint-set-repeat-mode-on-users-playback
 *
 * ----------------------------------------------------------------------------------------------------
 */
...
const run = async (input) => {
  ...
  try {
    ...
    const { data } = await axios({
      method: "put",
      url: `${SPOTIFY_BASE_URI}/me/player/repeat`,
      headers: {
