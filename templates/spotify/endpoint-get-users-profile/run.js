/**
 * ----------------------------------------------------------------------------------------------------
 * Get a User's Profile [Run]
 *
 * @description - Get a user's profile using the Spotify API
 *
 * @author    Buildable Technologies Inc.
 * @access    open
 * @license   MIT
 * @docs      https://developer.spotify.com/documentation/web-api/reference/#endpoint-get-users-profile
 *
 * ----------------------------------------------------------------------------------------------------
 */
...
const run = async (input) => {
  const { SPOTIFY_CLIENT_ID, SPOTIFY_CLIENT_SECRET, SPOTIFY_BASE_URI, user_id } = input;
  ...
  try {
    ...
    const { data } = await axios({
      method: "get",
      url: `${SPOTIFY_BASE_URI}/users/${user_id}`,
      headers: {
