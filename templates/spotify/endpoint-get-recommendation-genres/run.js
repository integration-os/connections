/**
 * ----------------------------------------------------------------------------------------------------
 * Get Recommendation Genres [Run]
 *
 * @description - Get recommendation genres using the Spotify API
 *
 * @author    Buildable Technologies Inc.
 * @access    open
 * @license   MIT
 * @docs      https://developer.spotify.com/documentation/web-api/reference/#endpoint-get-recommendation-genres
 *
 * ----------------------------------------------------------------------------------------------------
 */
...
const run = async (input) => {
  const { SPOTIFY_CLIENT_ID, SPOTIFY_CLIENT_SECRET, SPOTIFY_BASE_URI } = input;
  ...
  try {
    ...
    const { data } = await axios({
      method: "get",
      url: `${SPOTIFY_BASE_URI}/recommendations/available-genre-seeds`,
      headers: {
