/**
 * ----------------------------------------------------------------------------------------------------
 * Get User's Followed Artists [Run]
 *
 * @description - Get user's followed artists using the Spotify API
 *
 * @author    Buildable Technologies Inc.
 * @access    open
 * @license   MIT
 * @docs      https://developer.spotify.com/documentation/web-api/reference/#endpoint-get-followed
 *
 * ----------------------------------------------------------------------------------------------------
 */
...
const run = async (input) => {
  const { SPOTIFY_CLIENT_ID, SPOTIFY_CLIENT_SECRET, SPOTIFY_BASE_URI, type, after, limit } = input;
  ...
  try {
    ...
    const { data } = await axios({
      ...
      params: { type, ...(after ? { after } : {}), ...(limit ? { limit } : {}) },
    });
