/**
 * ----------------------------------------------------------------------------------------------------
 * Add an Item to Queue [Run]
 *
 * @description - Add an item to queue using the Spotify API
 *
...
const run = async (input) => {
  const { SPOTIFY_CLIENT_ID, SPOTIFY_CLIENT_SECRET, SPOTIFY_BASE_URI, uri, device_id } = input;
  ...
  try {
    ...
    const { data } = await axios({
      method: "post",
      url: `${SPOTIFY_BASE_URI}/me/player/queue`,
      headers: {
 ...
 */
const verifyInput = ({ SPOTIFY_CLIENT_ID, SPOTIFY_CLIENT_SECRET, SPOTIFY_BASE_URI, uri }) => {
  const ERRORS = {
