/**
 * ----------------------------------------------------------------------------------------------------
 * Get a Show's Episodes [Run]
 *
 * @description - Get a show's episodes using the Spotify API
 *
 * @author    Buildable Technologies Inc.
 * @access    open
 * @license   MIT
 * @docs      https://developer.spotify.com/documentation/web-api/reference/#endpoint-get-a-shows-episodes
 *
 * ----------------------------------------------------------------------------------------------------
 */
...
const run = async (input) => {
  const { SPOTIFY_CLIENT_ID, SPOTIFY_CLIENT_SECRET, SPOTIFY_BASE_URI, id, market, limit, offset } =
    input;
  ...
  try {
    ...
    const { data } = await axios({
      method: "get",
      url: `${SPOTIFY_BASE_URI}/shows/${id}/episodes`,
      headers: {
