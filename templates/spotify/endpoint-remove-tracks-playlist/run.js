/**
 * ----------------------------------------------------------------------------------------------------
 * Remove Items From a Playlist [Run]
 *
 * @description - Remove items from a playlist using the Spotify API
 *
...
const run = async (input) => {
  ...
  try {
    ...
    const { data } = await axios({
      method: "delete",
      url: `${SPOTIFY_BASE_URI}/playlists/${playlist_id}/tracks`,
      headers: {
        Authorization: `Bearer ${access_token}`,
      },
      data: { tracks, ...(snapshot_id ? { snapshot_id } : {}) },
    });
...
const verifyInput = ({
  ...
  const ERRORS = {
    ...
    INVALID_TRACKS: "A valid tracks field (object) was not provided in the input.",
  };
  ...
  if (typeof tracks !== "object") throw new Error(ERRORS.INVALID_TRACKS);
};
