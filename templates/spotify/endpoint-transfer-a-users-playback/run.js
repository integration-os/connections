/**
 * ----------------------------------------------------------------------------------------------------
 * Transfer a User's Playback [Run]
 *
 * @description - Transfer a user's playback using the Spotify API
 *
 * @author    Buildable Technologies Inc.
 * @access    open
 * @license   MIT
 * @docs      https://developer.spotify.com/documentation/web-api/reference/#endpoint-transfer-a-users-playback
 *
 * ----------------------------------------------------------------------------------------------------
 */
...
const run = async (input) => {
  const { SPOTIFY_CLIENT_ID, SPOTIFY_CLIENT_SECRET, SPOTIFY_BASE_URI, device_ids, play } = input;
  ...
  try {
    ...
    const { data } = await axios({
      method: "put",
      url: `${SPOTIFY_BASE_URI}/me/player`,
      headers: {
        Authorization: `Bearer ${access_token}`,
      },
      data: { device_ids, ...(play ? { play } : {}) },
    });
...
const verifyInput = ({
  ...
  const ERRORS = {
    ...
    INVALID_DEVICE_IDS: "A valid device_ids field (object) was not provided in the input.",
  };
  ...
  if (typeof device_ids !== "object") throw new Error(ERRORS.INVALID_DEVICE_IDS);
};
