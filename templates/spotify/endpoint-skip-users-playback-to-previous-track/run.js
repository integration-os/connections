/**
 * ----------------------------------------------------------------------------------------------------
 * Skip User’s Playback To Previous Track [Run]
 *
 * @description - Skip user’s playback to previous track using the Spotify API
 *
 * @author    Buildable Technologies Inc.
 * @access    open
 * @license   MIT
 * @docs      https://developer.spotify.com/documentation/web-api/reference/#endpoint-skip-users-playback-to-previous-track
 *
 * ----------------------------------------------------------------------------------------------------
 */
...
const run = async (input) => {
  const { SPOTIFY_CLIENT_ID, SPOTIFY_CLIENT_SECRET, SPOTIFY_BASE_URI, device_id } = input;
  ...
  try {
    ...
    const { data } = await axios({
      method: "post",
      url: `${SPOTIFY_BASE_URI}/me/player/previous`,
      headers: {
