/**
 * ----------------------------------------------------------------------------------------------------
 * Upload a Custom Playlist Cover Image [Run]
 *
 * @description - Upload a custom playlist cover image using the Spotify API
 *
 * @author    Buildable Technologies Inc.
 * @access    open
 * @license   MIT
 * @docs      https://developer.spotify.com/documentation/web-api/reference/#endpoint-upload-custom-playlist-cover
 *
 * ----------------------------------------------------------------------------------------------------
 */
...
const run = async (input) => {
  const { SPOTIFY_CLIENT_ID, SPOTIFY_CLIENT_SECRET, SPOTIFY_BASE_URI, playlist_id } = input;
  ...
  try {
    ...
    const { data } = await axios({
      method: "put",
      url: `${SPOTIFY_BASE_URI}/playlists/${playlist_id}/images`,
      headers: {
