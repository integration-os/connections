/**
 * ----------------------------------------------------------------------------------------------------
 * Update a Database [Run]
 *
 * @description - Update a database using the Notion API
 *
...
const run = async (input) => {
  const { NOTION_API_TOKEN, notionVersion, id, title, properties, ...rest } = input;
  ...
  try {
    const { data } = await axios({
      ...
      data: { ...(title ? { title } : {}), ...(properties ? { properties } : {}), ...rest },
    });
