 ...
 *
 * @description - Create a database using the Notion API
 *
...
const run = async (input) => {
  const { NOTION_API_TOKEN, notionVersion, parent, properties, title, ...rest } = input;
  ...
  try {
    const { data } = await axios({
      ...
      data: { parent, properties, ...(title ? { title } : {}), ...rest },
    });
